...
public class Controller
{
    ...
    public static Controller getInstance()
    {
        if (theOne == null)
        {
            ...
            try
            {
                ...
                theOne.mVideoGameDB = new DBModel(DB_NAME, VIDEO_GAME_TABLE_NAME, VIDEO_GAME_FIELD_NAMES,
                        VIDEO_GAME_FIELD_TYPES);
                ...
                theOne.mUserGamesDB = new DBModel(DB_NAME, USER_GAMES_TABLE_NAME, USER_GAMES_FIELD_NAMES,
    ...
    {
        if (!isValidEmail(email)) return "Email address is not valid.  Please use different address.";
    ...
    public ObservableList<String> getDistinctPlatforms()
    {
        ...
        for (VideoGame vg : theOne.mAllGamesList)
            if (!platforms.contains(vg.getPlatform())) platforms.add(vg.getPlatform());
    ...
    public ObservableList<String> getDistinctPublishers()
    {
        ...
        for (VideoGame vg : theOne.mAllGamesList)
            if (!publishers.contains(vg.getPublisher())) publishers.add(vg.getPublisher());
    ...
    public ObservableList<VideoGame> getGamesForCurrentUser()
    {
        ...
        if (mCurrentUser != null)
        {
            try
            {
                ArrayList<ArrayList<String>> resultsList = theOne.mUserGamesDB
                        .getRecord(String.valueOf(mCurrentUser.getId()));
                for (ArrayList<String> values : resultsList)
                {
                    int gameId = Integer.parseInt(values.get(1));
                    for (VideoGame vg : theOne.mAllGamesList)
                        if (vg.getId() == gameId) userGamesList.add(vg);
                }
    ...
    public boolean addGameToInventory(VideoGame selectedGame)
    {
        ...
        if (userGamesList.contains(selectedGame)) return false;
    ...
    private int initializeVideoGameDBFromFile() throws SQLException
    {
        ...
        if (theOne.mUserDB.getRecordCount() > 0) return 0;
        ...
        try
        {
            ...
            while (fileScanner.hasNextLine())
            {
                ...
                values[3] = data[4];
                values[4] = data[5];
                theOne.mVideoGameDB.createRecord(
                        Arrays.copyOfRange(VIDEO_GAME_FIELD_NAMES, 1, VIDEO_GAME_FIELD_NAMES.length), values);
                recordsCreated++;
            }
